...
This sim was scaffolded by `npm run scaffold-screens` into `src/reflection/`, `src/phase-relationships/`, `src/standing-waves/`, `src/instruments/` (fleet naming:
...
1. Copies the `reflection/` prototype into `src/<kebab>/` per screen
...
4. Removes the prototype `reflection/` folder
...
```
main.ts
  └─ ReflectionScreen            (Screen<ReflectionModel, ReflectionScreenView>)
       ├─ ReflectionModel         owns all state
       └─ ReflectionScreenView    owns all visuals
```
...
template's `ReflectionScreen.ts`. Screen icons live in one shared module under
...
```
src/common/StandingWavesScreenIcons.ts
```
